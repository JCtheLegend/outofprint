...
-- Lulu prints from two PDFs per book: the interior (already in books.pdf_url)
-- and the print-ready wraparound cover. Both live in the private `book-pdfs`
-- bucket and are handed to Lulu as signed URLs at print time.
alter table books add column if not exists cover_pdf_url text;

-- Lulu's product SKU (trim size, ink, quality, binding, paper, finish). Null
-- falls back to LULU_POD_PACKAGE_ID, which defaults to a 6x9 black-and-white
